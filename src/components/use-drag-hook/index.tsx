    ...
    if (dragRef?.current) {
      ...
      if (canScrollDirection !== "all") {
        if (canScrollDirection !== "up" && swipeDifference > 0) {
          swipeDifference = 0;
          return;
        } else if (canScrollDirection !== "down" && swipeDifference < 0) {
          swipeDifference = 0;
          return;
        }
      }
      ...
      if (swipeDifference > 0 && swipeDifference < stickDistance) {
        swipeDifference = 0;
      } else if (swipeDifference < 0 && swipeDifference > stickDistance * -1) {
        swipeDifference = 0;
      }

      (dragRef.current as HTMLDivElement).style.transform = `translateY(calc(${
        currentPageIndex * 100 * -1
      }dvh + ${swipeDifference * -1}px * -1))`;
      (dragRef.current as HTMLDivElement).style.transition = "transform 0.1s";
    }
  }
